...
- **Then**: チェックボックスは `region-notes.json` の `geo`（`jp` / `apac` / `eu` / `us` / `au` と、どれにも属さない `other`）でグループ分けされ、各行は「リージョンコード — 現地名（表示言語）」で並ぶ。グループの「すべて選ぶ」はそのグループの全リージョンを既存の選択に足し込む（他のグループの選択は消さない）
...
- 推論先の限定セレクタは見出しの無い 1 つのリスト（AC-018）。先頭が「制限なし」（既定）、
  ...
  2026-09-15 のデータでは次の 9 件になる:
  制限なし / 日本国内（2）/ 米国内（4）/ オーストラリア国内（2）/
