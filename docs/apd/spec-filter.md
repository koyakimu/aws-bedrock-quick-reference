...
- 国の選択肢も固定リストを持たず、`region-notes.json` の `country` の実値のうち**地理圏コードと同じ綴りのものだけ**を出す。地理圏コードは上の地理圏の一覧（`profiles.json` の `global` 以外の接頭辞 ∪ `region-notes.json` の `geo` の実値。`other` は除く）で、そこに現れない国コード（`nz` など）は選択肢にならない（`src/scripts/filter-model.mjs` の `countryCodes()`）
...
- **version 5** (2026-09-15): 地理圏の一覧を「接頭辞 5 種の固定リスト」から「`profiles.json` の `global` 以外の全接頭辞 と `region-notes.json` の `geo` の実値」に改め、辞書にラベルが無い地理圏はコードをそのまま出すことを AC-020 として明文化した。併せて `ca` / `in` のラベルを辞書に足し、カスタムのピッカーのグループ（AC-013）を `geo` の実値由来に改めた。`satisfiesLimit(destinations, L)` と AC-001 〜 AC-019 の判定内容は変更しない。理由: `ca.` / `in.` のプロファイルが実データに現れ、固定リストでは拾えなかった。AWS docs も地理圏の閉じた一覧を公開していない（D-012）
  - Notes に導出規則を追記 (2026-09-15): 国の選択肢が `region-notes.json` の `country` の実値のうち地理圏コードと一致するものだけになること（`countryCodes()`）。AC の判定内容は変更していない
